...
DAMNATION_JAMS = [12.5 for _ in range(5)]  # [5 for _ in range(10)]
...
# jams than targets)
SCORPION_JAMS = [8.5 for _ in range(0)]

# How many cycles will happen in the battle.
NUM_CYCLES = 18  # 90 cycles corresponds to a 30 minute fight
...
def single_cycle(jam_dist):
    active_jam_hits = sum([jam_result(x) for x in SCORPION_JAMS])
...
def distribute_lazy_jams():
    rand_jams = random.sample(DAMNATION_JAMS, len(DAMNATION_JAMS))
    i = 0
    for target_id in range(NUM_TARGETS):
